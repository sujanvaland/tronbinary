...
namespace SmartStore.Admin.Controllers
{
    ...
    public partial class SettingController : AdminControllerBase
	{
		...
		#region HYIP Settings
		[LoadSetting]
		public ActionResult Withdrawal(WithdrawalSettings withdrawalSettings)
		{
			if (!_services.Permissions.Authorize(StandardPermissionProvider.ManageSettings))
				return AccessDeniedView();

			var model = new WithdrawalSettingsModel();
			MiniMapper.Map(withdrawalSettings, model);
		...
		[HttpPost]
		[ValidateInput(false)]
		public ActionResult Withdrawal(WithdrawalSettings settings, WithdrawalSettingsModel model, FormCollection form)
		{
			if (!_services.Permissions.Authorize(StandardPermissionProvider.ManageSettings))
				return AccessDeniedView();
			...
			return NotifyAndRedirect("Withdrawal");
		}

		
		public ActionResult HyipPaymentSetting()
		{
			if (!_services.Permissions.Authorize(StandardPermissionProvider.ManageSettings))
				return AccessDeniedView();
		...
		[HttpPost]
		[ValidateInput(false)]
		public ActionResult HyipPaymentSetting(HyipPaymentSettingsModel model, FormCollection form)
		{
			if (!_services.Permissions.Authorize(StandardPermissionProvider.ManageSettings))
				return AccessDeniedView();
			...
			var coinpaymentSettings = _services.Settings.LoadSetting<CoinPaymentSettings>(storeScope);
			MiniMapper.Map(model.CoinPaymentSettings, coinpaymentSettings);

			var SolitTrustPaySettings = _services.Settings.LoadSetting<SolidTrustPaySettings>(storeScope);
			MiniMapper.Map(model.SolitTrustPaySettings, SolitTrustPaySettings);

			var PayzaSettings = _services.Settings.LoadSetting<PayzaSettings>(storeScope);
			MiniMapper.Map(model.PayzaSettings, PayzaSettings);

			var PMSettings = _services.Settings.LoadSetting<PMSettings>(storeScope);
			MiniMapper.Map(model.PMSettings, PMSettings);

			var PayeerSettings = _services.Settings.LoadSetting<PayeerSettings>(storeScope);
			MiniMapper.Map(model.PayeerSettings, PayeerSettings);

			using (Services.Settings.BeginScope())
			{
				StoreDependingSettings.UpdateSettings(coinpaymentSettings, form, storeScope, _services.Settings);
				StoreDependingSettings.UpdateSettings(SolitTrustPaySettings, form, storeScope, _services.Settings);
				StoreDependingSettings.UpdateSettings(PayzaSettings, form, storeScope, _services.Settings);
				StoreDependingSettings.UpdateSettings(PMSettings, form, storeScope, _services.Settings);
				StoreDependingSettings.UpdateSettings(PayeerSettings, form, storeScope, _services.Settings);
			}

			return NotifyAndRedirect("HyipPaymentSetting");
		}
		#endregion
	}
}
