	...
	public class CategoryImporter : EntityImporterBase
	{
			...
			Dictionary<int, ImportCategoryMapping> srcToDestId)
		{
			foreach (var row in batch)
			{
				...
				var rawplacedunderId = row.GetDataValue<string>("ParentCategoryId");
				var placedunderId = rawplacedunderId.ToInt(-1);

				if (id != 0 && placedunderId != -1 && srcToDestId.ContainsKey(id) && srcToDestId.ContainsKey(placedunderId))
				{
					// only touch hierarchical data if child and parent were inserted
					if (srcToDestId[id].Inserted && srcToDestId[placedunderId].Inserted && srcToDestId[id].DestinationId != 0)
					{
						var category = _categoryRepository.GetById(srcToDestId[id].DestinationId);
						if (category != null)
						{
							category.ParentCategoryId = srcToDestId[placedunderId].DestinationId;
